...
logfile     = options["logfile"].format(experimentID, arch, eps, alpha, num_iter, rand_loc, patch_size, num_poison, trigger_id)
lr            = float(options["lr"])
...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, is_inception=False):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test', 'notpatched', 'patched']:
            ...
            if phase == 'test':
            # if phase == 'patched' and (epoch_acc > best_acc):
...
# Poisoned dataset
saveDir = poison_root + "/" + experimentID + "/" + arch + "/rand_loc_" +  str(rand_loc) + "/eps_" + str(eps) +\
                    "/attack_iter_" + str(num_iter) + \
...
# checkpoint=torch.load("adv_train/0001/resnet/eps_8_alpha_1.0_iter_10_lr_0.001_epochs_30/adv_model.pt")
